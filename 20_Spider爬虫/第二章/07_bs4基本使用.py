...
# 拿到所有数据行
trs = table.find_all("tr")[1:]   # 过滤掉表头，表头不需要，只要数据
for tr in trs: # 每一行
    td = tr.find_all("td")  # 拿到每行中所有td
    name = td[0].text       # .text 表示拿到被标签暴击的内容
    low = td[1].text       # .text 表示拿到被标签暴击的内容
    avg = td[2].text       # .text 表示拿到被标签暴击的内容
    high = td[3].text       # .text 表示拿到被标签暴击的内容
    guige = td[4].text       # .text 表示拿到被标签暴击的内容
    kind = td[5].text       # .text 表示拿到被标签暴击的内容
    date = td[6].text       # .text 表示拿到被标签暴击的内容
